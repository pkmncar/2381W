...
WE CAN GET RID OF STUFF WE DO NOT NEED.
...
void trackPosition(int left, int right, int back, sPos& position)
{
	...
	//left, right, and back are the number of ticks that the wheels have rotated at the time that the funciton is called.
	//position.leftLst, etc. are the number of ticks that the wheel had moved the last time the function was called

	//this states how many inches the wheels have moved between when the code is running and when it last ran
...
void resetPositionFull(sPos& position, float y, float x, float a)
{
	...
	pros::lcd::set_text(1, "Resetting position");
  //pros::lcd::set_text(2, position.y, position.x, radToDeg(fmod(gPosition.a, PI * 2)), y, x, radToDeg(fmod(a, PI * 2)));
